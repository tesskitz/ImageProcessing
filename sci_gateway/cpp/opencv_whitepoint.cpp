...
extern "C"
{
  ...
  int opencv_whitepoint(char *fname, unsigned long fname_len)
  {
	...
	if(isStringType(pvApiCtx, piAddr1))
	{
       ...
       if(strcasecmp(pstData,"d65")==0)
       {
        output[0]=0.9504;
        output[1]=1.0000;
        output[2]=1.0889;
       }
      else if(strcasecmp(pstData,"d50")==0)
       {
        output[0]=0.9642;
        output[1]=1.0000;
        output[2]=0.8251;
       }
      else if(strcasecmp(pstData,"icc")==0)
       {
        output[0]=0.9642;
        output[1]=1.0000;
        output[2]=0.8249;  
       }
      else if(strcasecmp(pstData,"a")==0)
       {
        output[0]=1.0985;
        output[1]=1.0000;
        output[2]=0.3558;  
       } 
      else if(strcasecmp(pstData,"d55")==0)
       {
        output[0]=0.9568;
        output[1]=1.0000;
        output[2]=0.9214;   
       }
      else if(strcasecmp(pstData,"c")==0)
       {
        output[0]=0.9807;
        output[1]=1.0000;
        output[2]=1.1823; 
       ...
       {
         sciprint("The first argument must be  one of these strings:'icc', 'd50', 'd55', 'd65', 'c', 'a'");
         return 0;
       }
